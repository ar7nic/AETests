...
Every class exported from the `pages`, `components`, `asserts` and `assistants` barrels is
turned into a Playwright fixture automatically, keyed by whatever name the barrel gives it.
Adding a class to a barrel's exported registry object is the only step needed to make it
injectable in a spec; `fixtures/index.ts` itself never changes.
...
### Barrels export a name → class registry

Each barrel exports its classes individually **and** a plain object mapping the desired
fixture name to the class — the object key becomes the fixture name (no automatic
`ClassName` → `camelCase` derivation):
...
`components/index.ts`, `asserts/index.ts`, `assistants/index.ts` follow the same shape
(`components`, `asserts`, `assistants` respectively).

### Wiring fixtures from the registries
...
// flat merge of every barrel's registry; all registered classes share `new (page: Page) => T`
const registry = { ...pageObjects, ...components, ...asserts, ...assistants };

type AppFixtures = { [K in keyof typeof registry]: InstanceType<(typeof registry)[K]> };
...
export const test = base.extend<AppFixtures>(appFixtures as never);
export { expect } from '@playwright/test';
```

Each fixture instantiates its class directly from the raw `page` — there is no shared
per-test container object that pages/assistants read other fixtures out of. An assistant that
needs a page or component builds its own instance in its constructor instead (see
[page-object-model.md](page-object-model.md#assistants-behaviour-layer)).

Now any spec can request a page/component/assistant/assert by its registry name:

```typescript
import { test } from '../fixtures';

test('login @smoke', async ({ loginAssistant, commonAsserts, navBar }) => {
...
> Trade-off to be aware of: each fixture only depends on `page`, so per-class setup/teardown
> is not expressed here. Resources with a real lifecycle (auth, DB, API clients) use
> hand-written `test.extend()` fixtures layered on top — see below, and the real
> `registeredUser` / `isolateThirdParties` fixtures in `fixtures/index.ts` for a working
> example of each (a teardown-bearing precondition fixture, and an `auto: true` fixture).
