...
/ `element.label` off it) or the `page` the assert class was constructed with. This is the
only layer (besides the test framework itself) that touches `expect`.
...
test('home page loads', async ({ loginAssistant, commonAsserts, homePage }) => {
...
Add page-specific checks in their own assert classes; each is auto-registered as a fixture via
the `asserts/` barrel (see [fixtures-hooks.md](fixtures-hooks.md)) — no need to route
everything through `CommonAsserts`.
