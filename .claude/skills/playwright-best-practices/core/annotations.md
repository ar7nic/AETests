...
- **Do** put `@step(...)` (from `utils/step`) on every public **assistant** method, to group
  the auto-logged child steps under a readable heading (see the assistant example in
  [page-object-model.md](page-object-model.md)). Do not hand-roll a `test.step(...)` call
  inside the method body instead — the decorator *is* the step.
- **Reuse values returned by reader steps.** `Element` readers (currently `innerText()`)
  already run inside their own `test.step` and return the value — assign it directly instead
  of wrapping the call in another step:
  ```typescript
  const heading = await accountCreatedPage.heading.innerText();  // step logged automatically
  await commonAsserts.toContainText(navBar.loggedInAs, heading);
  ```
...
Page objects hold locators only and must **not** contain `test.step`. Group a flow under one
step in an **assistant** instead, using the `@step` method decorator — the per-action steps
from `Element` nest beneath it automatically:
...
`@step` accepts a static title (as above), a resolver built from the call args
(`@step((id: string) => \`Open order "${id}"\`)`), or nothing (defaults to the method name).
Reference implementation: `utils/step.ts`.
