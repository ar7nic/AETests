...
| **Assistant** | Multi-step / cross-page flows | Built per test (constructed with `page`) | Behaviour |
...
export class BookingPage extends BasePage {
  protected readonly PAGE_NAME = 'Booking';

  readonly dateField: Element = this.el('Check-in date', this.page.getByLabel('Check-in date'));
  readonly guestCount: Element = this.el('Guests', this.page.getByLabel('Guests'));
  readonly roomType: Element = this.el('Room type', this.page.getByLabel('Room type'));
  readonly reserveBtn: Element = this.el(
    'Reserve button',
    this.page.getByRole('button', { name: 'Reserve' }),
  );
  readonly totalPrice: Element = this.el('Total price', this.page.getByTestId('total-price'));
}
```
...
export class BookingAssistant {
  ...
  @step('Booking → reserve standard room')
  async reserveStandardRoom(date: string, guests: number): Promise<void> {
    await this.page.goto('/booking');
    await this.bookingPage.dateField.fill(date);
    await this.bookingPage.guestCount.fill(String(guests));
    await this.bookingPage.roomType.selectOption('standard');
    await this.bookingPage.reserveBtn.click();
...
test('complete reservation with standard room', async ({
  bookingAssistant,
  commonAsserts,
  bookingPage,
}) => {
  await bookingAssistant.reserveStandardRoom('2026-03-15', 2);
  await commonAsserts.toBeVisible(bookingPage.totalPrice);
});
```
...
| **Assistants** | Classes (constructed with `page`) | Multi-step / cross-page flows |
